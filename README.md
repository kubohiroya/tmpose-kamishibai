...
アプリの操作、台本DSL、アクション、ポーズ認識、作品作成手順は、[利用ガイド](docs/index.md)と[親子AIプログラミング体験会教材](docs/tmpose-kamishibai-textbook-20260801.md)のMarkdown原稿で管理しています。教材の目次はMarkdownへ固定せず、Vivliostyleが見出しからHTML/PDF用に自動生成します。生成パイプラインは次の順に処理します。
...
└── docs/
    ...
    └── textbook/
        ├── index.html
        ├── publication.json
        ├── tmpose-kamishibai-textbook-20260801.html
        └── tmpose-kamishibai-textbook-20260801.pdf
...
│   ├── textbook-theme.css
│   ├── theme.css
│   ├── tmpose-kamishibai-textbook-20260801.md
│   └── vivliostyle.textbook.config.mjs
├── scripts/
│   ├── build-docs.mjs
│   ├── build-site.mjs
│   └── verify-build.mjs
