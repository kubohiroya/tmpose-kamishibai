...
現在の配布用アプリは、コマンド行を半角 `=` で分割し、2番目の要素だけを値として読み取ります。このため、`score == 1`、`score != 1`、`score === 1`、`score !== 1` のように `=` を含む条件式は途中で切れ、正しく登録されません。条件には、上の例の `takeSeaRoute` のように真偽値を保持したランタイム変数を直接指定します。
...
| `=` はコマンドの区切り | 現在の配布用アプリは、1番目をキー、2番目を値として読み取ります。値にさらに `=` があると、それ以降は保持されません。 |
